                ...
                <div class="col-12">
                    <div class="row">
                        <div class="col-4">
                            ...
                            <input type="number" value="0" min="0"  class="form-control" name="age_year" placeholder="number of age years" required/>
                        </div>
                        <div class="col-4">
                            <label class="form-label">Number of age months</label>
                            <input type="number" value="0" min="0" max="12" class="form-control" name="age_month" placeholder="number of age months" required/>
                        </div>
                        <div class="col-4">
                            <label class="form-label">Number of age days</label>
                            <input type="number" value="0" min="0" max="30" class="form-control" name="age_day" placeholder="number of age days" required/>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                @foreach ((new ReflectionClass(App\Enums\GenderEnum::class))->getConstants() as $type)
                    <label class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="gender" value="{{$type}}" {{ $type == App\Enums\GenderEnum::FEMALE ? 'checked' : '' }}>
        ...
        <div class="mb-3 col-md-6">
            ...
            <select class="form-select" name="role">
                    @foreach ((new ReflectionClass(App\Enums\UserRoleEnum::class))->getConstants() as $role)
                        <option value="{{$role}}" {{ $role == App\Enums\UserRoleEnum::VISITOR ? 'selected' : '' }}>{{$role}}</option>
                    @endforeach
            </select>
        </div>
