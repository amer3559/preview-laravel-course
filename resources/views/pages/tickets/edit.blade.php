             ...
             <div class="mb-3 col-md-12">
                 ...
                 <select class="form-select" name="patient_id" id="userId" required>
                     @foreach ($visitors as $visitor)
                         <option value="{{$visitor->id}}" {{ $ticket->patient_id == $visitor->id ? 'selected' : '' }} >{{$visitor->name}}</option>
                     @endforeach
                 </select>
             </div>
             <div class="mb-3 col-md-12">
                     ...
                     @foreach ((new ReflectionClass(App\Enums\AttendanceEnum::class))->getConstants() as $type)
                         <label class="form-check form-check-inline">
                             <input class="form-check-input" type="radio" name="attendance" value="{{$type}}"
                                 {{ $type == $ticket->attendance ? 'checked' : '' }}>
             ...
             <div class="mb-3 col-md-12">
                 ...
                 <select class="form-select" name="doctor_id" required>
                     @foreach ($doctors as $doctor)
                         <option value="{{$doctor->id}}" {{ $doctor->id == $ticket->doctor_id ? 'selected' : '' }} >{{$doctor->name}}</option>
                     @endforeach
                 </select>
             </div>
             <div class="mb-3 col-md-6">
                 <label class="form-label">Number</label>
                 <input type="text" class="form-control" name="number" value="{{$ticket->number}}"  required/>
             </div>
             <div class="mb-3 col-md-6">
